...
public static class TrueColorConverter
{
    ...
    /// <summary>24-bit foreground SGR from an RGB <see cref="Color"/>.</summary>
    public static string Foreground(Color color)
        => ToForegroundSequence(color);

    /// <summary>24-bit background SGR from an RGB <see cref="Color"/>.</summary>
    public static string Background(Color color)
        => ToBackgroundSequence(color);
    ...
    private static string ToForegroundSequence(Color c)
        => $"\x1b[38;2;{c.R};{c.G};{c.B}m";

    private static string ToBackgroundSequence(Color c)
